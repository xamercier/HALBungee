...
import ca.xamercier.hal.bungee.Games.Utils.HikaBrain2v2Utils;
 ...
 */
public class HikaBrain2v2Thread extends Thread {
	...
	public void run() {
		...
		while (true) {
			for (ProxiedPlayer p : HALBungee.waitingLine.keySet()) {
				if (HALBungee.waitingLine.get(p).equalsIgnoreCase("HikaBrain2v2")) {
			...
			while (entries.hasNext()) {
				...
				String srvType;
				String srvPort;
				if (name.contains("HikaBrain2v2_")) {
					String[] nameANDport;
			...
			/*
			 * System.out.println("Hikabrain2v2:  StartGames:" +
			...
			if (DisponibleGames.size() == 0 && NotReadyGames.size() == 0) {
				...
				startServer.put("serverType", "HikaBrain2v2");
			...
			if (DisponibleGames.size() > minNumber) {
				if (HALBungee.getInstance().getSQL().getPlayers(Integer.parseInt(DisponibleGames.get(0))) > 0) {
					String newServer = HikaBrain2v2Utils.getCompEmptyGame();
					for (ProxiedPlayer player : HALBungee.getInstance().getProxy().getServers()
							.get("HikaBrain2v2_" + DisponibleGames.get(0)).getPlayers()) {
						if (newServer == null) {
							player.sendMessage(ChatColor.RED
									+ "Les serveurs de HikaBrains2v2 sont en surcharge ressaie dans quelques instants");
