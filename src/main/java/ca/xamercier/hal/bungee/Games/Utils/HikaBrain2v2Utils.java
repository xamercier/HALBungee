 ...
 */
public class HikaBrain2v2Utils {
	...
	public static String getCompGame(ProxiedPlayer p) {
		...
		for (String name : CompGames) {
			...
			srvType = nameANDport[0];
			srvPort = nameANDport[1];
			if(HALBungee.getInstance().getProxy().getServerInfo(srvType + "_" + srvPort).getPlayers().size() == 3) {
				ReturnedGames.add(name);
				return name;
			}
			if(HALBungee.getInstance().getProxy().getServerInfo(srvType + "_" + srvPort).getPlayers().size() == 2) {
				ReturnedGames.add(name);
				return name;
			}
